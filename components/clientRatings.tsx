...
const ClientRatings = ({ contract }: { contract: Contract }) => {
  ...
  const uploadReview = async (rating: number, review: string) => {
    try {
      if (!jobId || !contractId || !clientId || rating === 0 || review === "") {
        return toast.error(
      ...
      <Dialog>
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            disabled={contract?.clientReviews?.length > 0}
          >
            {contract?.clientReviews?.length > 0
              ? "Already rated"
              : "Leave feedback"}
            {contract?.clientReviews?.length > 0 &&
              contract.clientReviews[0].rating && (
                <span className="text-sm text-gray-500">
                  <Rating
                    style={{ maxWidth: 120 }}
                    value={contract.clientReviews[0].rating}
                    readOnly={true}
                  />
                </span>
              )}
